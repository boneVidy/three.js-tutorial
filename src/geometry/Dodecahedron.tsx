...
import {
    ...
    DirectionalLight, DodecahedronGeometry, DoubleSide,
    FrontSide,
    Line,
    LineBasicMaterial,
    Mesh,
...
function Dodecahedron() {
    useEffect(() => {
        ...
        const dodecahedronGeometry = new DodecahedronGeometry(100,32);
        const meshLambertMaterial = new MeshLambertMaterial({
            color: 0xabcd33,
            side: DoubleSide
        });
        const cube = new Mesh(dodecahedronGeometry, meshLambertMaterial);
        // cube.position.x = 100;
        // cube.position.y = 3;
        scene.add(cube);
